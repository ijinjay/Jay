...
char *condition(int *i){
    ...
    if (wordIndex[*i] == 7) {
        ...
        char *elseStr = semanticAnalysis(i);
        sprintf(temp, "%s(%d)\n%s(%d)\tgoto\t(%d)\n%s",fChain,falseIndex+1,semanStr,falseIndex,finalIndex,elseStr);
        ifNumber --;
        (*i) += 2;
    }
    else {
...
char *semanticAnalysis(int *i){
    ...
    while(wordIndex[*i] != 29) {
        char *tmp = (char *)malloc(sizeof(char)*BUFSIZ);
        switch (wordIndex[*i]) {
            case 10:tmp = assignment(i);break;
            case 6:tmp = condition(i);break;
            case 8:tmp = doloop(i);break;
            default:
                break;
        }
        if (tmp != NULL) {
